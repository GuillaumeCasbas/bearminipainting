...
describe('CodeNotUniqueError', () => {
    it('should create error with code property', () => {
        ...
        const error = new project_errors_1.CodeNotUniqueError(code);
        expect(error).toBeInstanceOf(Error);
        expect(error).toBeInstanceOf(project_errors_1.CodeNotUniqueError);
        expect(error.code).toBe(code);
        expect(error.name).toBe('CodeNotUniqueError');
        expect(error.message).toContain(code);
    });
    it('should have readonly code property', () => {
        const error = new project_errors_1.CodeNotUniqueError('TEST-001');
