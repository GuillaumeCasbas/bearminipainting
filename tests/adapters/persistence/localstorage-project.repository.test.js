...
const project_repository_1 = require("../../../src/adapters/persistence/localstorage/project.repository");
...
describe('LocalStorageProjectRepository Integration', () => {
    ...
    beforeEach(() => {
        ...
        jest.clearAllMocks();
        repository = new project_repository_1.LocalStorageProjectRepository();
        useCase = new create_project_usecase_1.CreateProjectUseCase(repository);
    });
    it('should save and find a project by code', async () => {
        const project = new Project_1.Project('test-id-1', 'Test Project', 'TEST-001', []);
    ...
    it('should integrate with CreateProjectUseCase and throw on duplicate code', async () => {
        ...
        await expect(useCase.execute('Second Project', 'DUPLICATE-001'))
            .rejects
            .toThrow(project_errors_1.CodeNotUniqueError);
    });
    it('should update existing project when saving with same id', async () => {
        const project1 = new Project_1.Project('same-id', 'Original Name', 'CODE-001', []);
        const project2 = new Project_1.Project('same-id', 'Updated Name', 'CODE-001', []);
    ...
    it('should return all projects with findAll', async () => {
        const project1 = new Project_1.Project('id-1', 'Project One', 'CODE-001', []);
        const project2 = new Project_1.Project('id-2', 'Project Two', 'CODE-002', []);
        const project3 = new Project_1.Project('id-3', 'Project Three', 'CODE-003', []);
        await repository.save(project1);
        await repository.save(project2);
    ...
    it('should save and find a project with units', async () => {
        const unit = new Unit_1.Unit('unit-1', 'Intercessor', 'IA-01', 'test-id-1');
        const project = new Project_1.Project('test-id-1', 'Test Project', 'TEST-001', [unit]);
        await repository.save(project);
