...
export class AddTodoToUnitUseCase {
  ...
  async execute(unitId: string, label: string): Promise<Unit> {
    ...
    const lastOrder = unit.todos.length > 0 ? Math.max(...unit.todos.map((todo) => todo.order)) : 0;
    const newOrder = lastOrder + 10;

    // Create the new todo
    const newTodo = new Todo(crypto.randomUUID(), label.trim(), 'TODO', newOrder);

    // Create a new unit instance with the updated todos
    const updatedUnit = new Unit(unit.id, unit.name, unit.code, unit.projectId, [
      ...unit.todos,
      newTodo,
    ]);
